...
GitHub Actions 的 `.github/workflows/ci.yml` 会在 push 和 pull request 上运行不需要真实 CloudBase secret 的自动门禁：lint、typecheck、云函数 test/typecheck/build/smoke、单元与安全测试、production build、真实 smoke 模板脱敏校验、产物扫描和 test-platform 移动端 E2E。真实 `pnpm preflight:cloudbase-manual` 与 CloudBase manual smoke 需要隔离环境变量、测试邮箱和大陆网络设备，必须由发布人按本文档手动执行，不得在 CI 中放入服务端模型 secret。
