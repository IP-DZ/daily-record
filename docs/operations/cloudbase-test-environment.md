...
6. 执行默认自动证据：
   ...
   `pnpm preflight:cloudbase-manual` 只检查变量是否齐全、地域和模型配置是否合法、公开 `VITE_CLOUDBASE_*` 与云函数 `CLOUDBASE_*` 是否指向同一个隔离环境/地域，以及是否把服务端 secret 错放进 `VITE_*` 公开变量；它只输出变量名与 pass/fail，不输出实际 env id、key、endpoint 或 secret。
